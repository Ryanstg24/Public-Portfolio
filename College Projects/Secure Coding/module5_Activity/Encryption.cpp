...
std::string encrypt_decrypt(const std::string& source, const std::string& key)
{
  ...
  { // xor the source char with the key char
...
{
  // save the encrypted string to a file with the student name as the filename
  std::ofstream file(filename);
    ...
    if (file) {
        ...
        file << student_name << "\n";
        file << std::string(dt).substr(0, 10) << "\n"; // YYYY-MM-DD format
        file << key << "\n";
        file << data;
