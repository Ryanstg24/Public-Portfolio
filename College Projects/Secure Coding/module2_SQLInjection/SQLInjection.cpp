...
{
  // Converted to lower and checked to detect "OR value=value" pattern
  std::string lower_sql = sql;
  std::transform(lower_sql.begin(), lower_sql.end(), lower_sql.begin(), ::tolower);

  // Regular expression to detect "OR value=value" pattern where value is a numeric or quoted string
  std::regex injection_pattern(R"(or (\d+|\'.+?\')=\1)");

  if (std::regex_search(lower_sql, injection_pattern))
  {
    std::cout << "Suspected SQL Injection detected! Aborting query." << std::endl;
    return false;
  }
